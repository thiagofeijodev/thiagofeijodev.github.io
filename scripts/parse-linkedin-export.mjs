...
import { extractZip } from './utils/extractZip.mjs';
import { normalizeDate } from './utils/normalizeDate.mjs';
import { parseShareDate } from './utils/parseShareDate.mjs';
import { readCSV } from './utils/readCSV.mjs';
import { splitCommentary } from './utils/splitCommentary.mjs';
...
async function main() {
  ...
  if (!input) {
    console.error('Usage: node scripts/parse-linkedin-export.mjs <path-to-export-folder-or-zip>');
    process.exit(1);
  }
  ...
  const pdfScript = resolve(__dirname, 'generate-cv-pdf.mjs');
